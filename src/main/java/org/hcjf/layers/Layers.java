    ...
    public static synchronized <L extends Layer> String publishLayer(L layerInstance) {
        ...
        while(!classToIntrospect.equals(Layer.class) && !classToIntrospect.equals(Object.class)) {
            for (Method method : classToIntrospect.getDeclaredMethods()) {
                clazz = layerInstance.getClass();
                if (method.getDeclaredAnnotationsByType(SuperClassPermission.class).length > 0) {
                    clazz = method.getDeclaredAnnotation(SuperClassPermission.class).value();
                }
                for (Permission permission : method.getDeclaredAnnotationsByType(Permission.class)) {
                    SecurityPermissions.publishPermission(clazz, permission.value(), permission.title(), permission.description(), List.of(permission.tags()));
                }
