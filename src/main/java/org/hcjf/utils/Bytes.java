...
public class Bytes {
    ...
    private static final byte[] HUFFMAN_CODE_LENGTHS = {
    ...
    private static final int HUFFMAN_EOS = 256;
    ...
    public static ByteBuffer encodeHuffman(ByteBuffer data) {
        ByteBuffer result = ByteBuffer.allocate(getHuffmanEncodedLength(data));
        long current = 0;
        int n = 0;

        while(data.position() < data.limit()) {
            int b = data.get() & 0xFF;
            int code = HUFFMAN_CODES[b];
            int nbits = HUFFMAN_CODE_LENGTHS[b];
        ...
        if (n > 0) {
            current <<= (8 - n);
            current |= (0xFF >>> n);
            result.putInt((int)current);
        }

        return result;
    }

    public static Integer getHuffmanEncodedLength(ByteBuffer data) {
        long len = 0;
        while(data.position() < data.limit()) {
            len += HUFFMAN_CODE_LENGTHS[data.get() & 0xFF];
        }
        return (int)((len + 7) >> 3);
    }
}
