...
public enum HttpMethod {
     ...
     * applicable to that resource (e.g., Allow), possibly including extensions not defined by this specification.
     * The response body, if any, SHOULD also include information about the communication options. The format for such a
     * body is not defined by this specification, but might be defined by future extensions to HTTP. Content negotiation
     * MAY be used to select the appropriate response format. If no response body is included, the response MUST include
     * a Content-Length field with a field-value of "0".
     * The Max-Forwards request-header field MAY be used to target a specific proxy in the request chain. When a proxy
     * receives an OPTIONS request on an absoluteURI for which request forwarding is permitted, the proxy MUST check for
     * a Max-Forwards field. If the Max-Forwards field-value is zero ("0"), the proxy MUST NOT forward the message;
     * instead, the proxy SHOULD respond with its own communication options. If the Max-Forwards field-value is an
     * integer greater than zero, the proxy MUST decrement the field-value when it forwards the request. If no
     * Max-Forwards field is present in the request, then the forwarded request MUST NOT include a Max-Forwards field
     */
    OPTIONS,
     ...
     * A partial GET requests that only part of the entity be transferred, as described in section 14.35. The partial
     ...
     * be used to update a previously cached entity from that resource. If the new field values indicate that the cached
     ...
     * subordinate to a database. The action performed by the POST method might not result in a resource that can be
     * identified by a URI. In this case, either 200 (OK) or 204 (No Content) is the appropriate response status,
     ...
     * That resource might be a data-accepting process, a gateway to some other protocol, or a separate entity that
     * accepts annotations. In contrast, the URI in a PUT request identifies the entity enclosed with the request -- the
     * user agent knows what URI is intended and the server MUST NOT attempt to apply the request to some other
     * resource. If the server desires that the request be applied to a different URI, it MUST send a 301
     * (Moved Permanently) response; the user agent MAY then make its own decision regarding whether or not to redirect
     * the request.
     ...
     * since it acts as a trace of the request chain. Use of the Max-Forwards header field allows the client to limit
     * the length of the request chain, which is useful for testing a chain of proxies forwarding messages in an
     * infinite loop.
